...
(function($) {
  ...
  $.fn.jPaginate = function(option) {
    ...
    if (option.itemClass == '') {
      handle.append('<p class="jq-pagination-error">itemClass not defined</p>');
    ...
    if (isAjax) {
      ...
      if (count == 0) {
        handle.append('jQueryPagination error: items with class "' + option.itemClass + '" not found.');
        return false;
      }
    ...
    function render() {
      var currPage = Math.floor(option.offset - option.perPage) / option.perPage + 2;
      ...
      if (isAjax) {
        $.ajax({
          ...
          success: function(res) {
            ...
            if (items.length == 0) {
              handle.append('<p class="jq-pagination-error">jPaginate error: items with class "' + option.itemClass + '" not found.</p>');
              return false;
            }
    ...
    function navLinks() {
      var nav = $('.jq-pagination-nav', handle),
        i, j, title;
      ...
      if (option.navLinks == 'max') {
        if (option.maxPageLinks == 0 || numPages <= option.maxPageLinks) {
          ...
          var maxlc = Math.ceil(option.maxPageLinks / 2);
          var maxlf = Math.floor(option.maxPageLinks / 2);
          var left = (currPage - maxlf) > 0 ? currPage - maxlf : 1;
          var right = (currPage + maxlc) < numPages ? currPage + maxlc - 1 : numPages;
